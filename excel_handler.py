...
class ExcelHandler:
    ...
    def get_preview(self, rows=5):
        """Get a preview of the loaded Excel data"""
        if self.current_df is None:
            return "No Excel file loaded"
        return str(self.current_df.head(rows))
    ...
    def get_batch_data(self, batch_size=10):
        ...
        for start_idx in range(0, total_rows, batch_size):
            end_idx = min(start_idx + batch_size, total_rows)
            batch = self.current_df.iloc[start_idx:end_idx]
            batches.append(batch)
    ...
    def save_processed_status(self, processed_rows, status_col='Processed', timestamp_col='Processed_Time'):
        ...
        try:
            # Create backup of original file
            backup_file = f"backup_{datetime.now().strftime('%Y%m%d%H%M%S')}.xlsx"
            self.current_df.to_excel(backup_file, index=False)
            self.logger.info(f"Created backup of Excel file: {backup_file}")
            
            # Add status columns if they don't exist
            if status_col not in self.current_df.columns:
                self.current_df[status_col] = ''
            if timestamp_col not in self.current_df.columns:
                self.current_df[timestamp_col] = ''
            
            # Update status for processed rows
            timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            for row_idx in processed_rows:
                if 0 <= row_idx < self.current_df.shape[0]:
                    self.current_df.at[row_idx, status_col] = 'Processed'
                    self.current_df.at[row_idx, timestamp_col] = timestamp
            
            # Save updated file
            self.current_df.to_excel("updated_file.xlsx", index=False)
            self.logger.info(f"Updated processing status in Excel file: updated_file.xlsx")
