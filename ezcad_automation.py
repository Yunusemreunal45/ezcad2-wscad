...
def _send_command(self, command):
    ...
    def _process_excel(self):
        ...
        self.logger.info(f"Adding Excel processing job: {excel_file}")
        
        # Add job to the queue
        job_id = self.queue_manager.add_job(excel_file, 'excel')
        
        # Make sure processing is started
        if not self.queue_manager.should_run:
            self.queue_manager.start_processing()
        
        messagebox.showinfo("Job Added", f"Excel processing job added with ID: {job_id}")
